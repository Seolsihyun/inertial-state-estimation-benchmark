...
class RotationErrorStateKalmanFilter15D:
    ...
    def __init__(
        ...
        if pose_type == "6d":
            pose_type = "3d"
        if pose_type != "3d":
            raise ValueError("RotationErrorStateKalmanFilter15D supports only 3d pose.")
    ...
    @classmethod
    def from_configs(cls, dataset_config: dict, compare_config: dict) -> "RotationErrorStateKalmanFilter15D":
        cfg = compare_config.get("eskf_15d", compare_config.get("inertial_ekf_15d", compare_config.get("inertial_extended_kalman_filter_15d", compare_config)))
        return cls(
    ...
    def predict(self, control: Iterable[float] | None, dt: float) -> np.ndarray:
        ...
        if u.size < 6:
            raise ValueError("3D ESKF control must contain [ax, ay, az, gx, gy, gz].")
...
# Backward-compatible alias for old experiment scripts.
ErrorStateKalmanFilter15D = RotationErrorStateKalmanFilter15D
InertialExtendedKalmanFilter15D = RotationErrorStateKalmanFilter15D
RotationESKF15D = RotationErrorStateKalmanFilter15D

# Short name used by the runners and filter registry.
ESKF = RotationErrorStateKalmanFilter15D
__all__ = ["ESKF", "RotationErrorStateKalmanFilter15D", "ErrorStateKalmanFilter15D"]
